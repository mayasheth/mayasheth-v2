...
function interpolateMissing(sizeToGrams) {
  ...
  const sizes = Object.keys(sizeToGrams)
    .map(Number)
    .sort((a, b) => a - b);
  ...
  for (let i = 0; i < sizes.length; i++) {
    if (sizeToGrams[sizes[i]] === null) {
      ...
      // Find next known
      let nextIdx = i + 1;
      while (nextIdx < sizes.length && sizeToGrams[sizes[nextIdx]] === null)
        nextIdx++;
      // If both bounds exist, interpolate
      if (prevIdx >= 0 && nextIdx < sizes.length) {
        const sizeA = sizes[prevIdx],
          gramsA = sizeToGrams[sizeA];
        const sizeB = sizes[nextIdx],
          gramsB = sizeToGrams[sizeB];
...
async function getCarbonGrams(bytes, green = 0, retryCount = 0) {
  try {
    ...
    if (!res.ok || !contentType.includes("application/json")) {
      ...
      console.error(
        `Carbon API HTTP error [size ${bytes}]: ${res.status}\nContent-Type: ${contentType}\nMessage: ${text.slice(0, 120)}...`,
      );
    ...
    try {
      ...
      const text = await res.text();
      console.error(
        `Carbon API response for size ${bytes} failed JSON parse. Content-Type: ${contentType}\nSample: ${text.slice(0, 100)}...`,
      );
      return null;
    }
